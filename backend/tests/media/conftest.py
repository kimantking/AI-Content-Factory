...
def tmp_storage(tmp_path, _base_settings):
    ...
    # get_storage() is lru_cache'd — clear it so it picks up the tmp roots
...
def ready_campaign(_base_settings, tmp_storage):
    """A campaign that has completed Phase 1-A, ready for the media pipeline."""
    cid = str(uuid.uuid4())
    platforms = ["youtube_shorts", "instagram_carousel", "threads"]
