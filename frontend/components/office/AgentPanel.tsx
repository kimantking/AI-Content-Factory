...
// Pending requests survive closing a panel, but cannot survive a page reload.
// Keep them in memory so a stale storage flag never locks the composer.
const pendingChats = new Set<AgentId>();
const chatErrors = new Map<AgentId, string>();
...
export function AgentPanel({
  ...
  useEffect(() => {
    ...
    try {
      ...
      setSending(pendingChats.has(id));
    } catch {
      setMessages([greeting]);
    }
    setDraft("");
    setChatError(chatErrors.get(id) ?? "");
    setChatMeta(null);
  }, [id, agent.role]);
  ...
  async function sendText(text: string) {
    text = text.trim();
    if (!text || sending || pendingChats.has(id)) return;
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "AI 연결에 실패했습니다. 설정을 확인하고 다시 시도해 주세요.";
      chatErrors.set(id, message);
  ...
  return (
    <aside
      ref={panelRef}
      aria-modal="true"
      className="pointer-events-auto relative isolate flex h-[100dvh] max-h-none w-full flex-col gap-3 overflow-y-auto border border-hairline bg-surface-1 p-4 shadow-[0_24px_70px_rgba(47,22,37,.32)] backdrop-blur-xl sm:h-[min(760px,calc(100vh-2rem))] sm:w-[min(720px,calc(100vw-3rem))] sm:rounded-2xl sm:p-6"
      role="dialog"
      aria-label={`${agent.name} 상세`}
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-3">
          <div className="h-14 w-14 shrink-0 overflow-hidden rounded-2xl bg-[#f4e6ef]">
            <img src={`/agent-${id}.webp`} alt="" className="w-full" />
          </div>
          <div><p className="font-mono text-[11px] uppercase tracking-[0.14em] text-primary">{agent.role}</p>
      ...
      {model.hasJob ? (
        <details className="shrink-0 rounded-xl border border-hairline p-3"><summary className="cursor-pointer truncate text-sm text-ink-muted">현재 작업 · {j.topic ?? "상세 보기"}</summary><dl className="mt-2 max-h-36 space-y-0 overflow-auto">
      ...
      <div className="flex min-h-0 flex-1 flex-col border-t border-hairline pt-3">
        <div className="mb-2 flex items-center justify-between gap-2">
          ...
          <button type="button" onClick={clearChat} disabled={sending} className="text-[12px] text-ink-tertiary hover:text-primary">대화 초기화</button>
        </div>
        ...
        <div role="log" aria-label="대화 내용" aria-live="polite" className="min-h-[120px] flex-1 space-y-3 overflow-y-auto overscroll-contain rounded-xl border border-primary/10 bg-surface-2/65 p-3 sm:min-h-[160px] sm:p-4">
          {messages.map((message, index) => (
            <div key={index} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`rounded-xl px-3.5 py-3 sm:px-4 ${message.role === "user" ? "max-w-[88%] bg-primary text-[15px] leading-6 text-white" : "w-full bg-surface-1 shadow-sm ring-1 ring-black/[0.04]"}`}>
        ...
        {chatError && <div role="alert" className="mt-2 rounded-xl border border-red-300/50 p-3 text-xs text-red-600">
          <p>{chatError}</p>
          <button type="button" disabled={sending} className="mt-2 font-semibold underline" onClick={() => {
        ...
        <form onSubmit={sendMessage} className="mt-3 flex items-end gap-2">
          <textarea ref={draftRef} aria-label="에이전트에게 보낼 메시지" value={draft} onChange={(e) => setDraft(e.target.value)} className="input min-h-[48px] min-w-0 flex-1 resize-none !bg-surface-1 py-3 text-[16px] leading-6" onKeyDown={(e) => {
            if (e.key === "Enter" && !e.shiftKey && !e.nativeEvent.isComposing) { e.preventDefault(); void sendText(draft); }
          }} placeholder="질문을 입력하세요 · Shift+Enter 줄바꿈" maxLength={4000} rows={2} />
      ...
      <p className="text-[11px] text-ink-subtle">대화는 제작 조언입니다. 작업 실행·변경은 제작 화면에서 진행하세요. 기록은 이 브라우저에 저장됩니다.</p>
